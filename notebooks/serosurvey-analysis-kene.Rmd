...
n_chains <- 4
# kene: run fewer iterations. Original was 1500. try 15 instead. 
# warmup was 250. try 3 insteas
# n_iter <- 1500
# n_warmup <- 250
...
plots_over_time <- map(
  ...
  .f = function(column) {
    inc_dat %>% 
      ggplot(aes_string(x = "date", y = column)) +
      geom_col(fill = "dodgerblue3") + 
      scale_x_date(date_labels = "%b %d") +
      labs(title = glue("{column} over time") ) + 
      theme(title = element_text(size = 7))
  }
)

patchwork::wrap_plots(plots_over_time)
