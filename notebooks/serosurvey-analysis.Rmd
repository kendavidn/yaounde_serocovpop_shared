...
source("scripts/utils_geneva_study.R")
...
# positive samples from validation study
pos_control <- 82 

# true positives detected among the positives
control_tp_hi <- 75

# negative samples from validation study
neg_control <- 150

# false positives detected among the negatives
control_fp_hi <- 1 
```

```{r epi data, include = FALSE}
## Geneva ages
yao_ages <- read_excel("data/yaounde_age_2018_2019.xlsx") %>%
  mutate_at(
    c("men", "women", "total"),
...
sero_dat <- read_csv("generated_data/serocov-pop_data_yaounde.csv") %>%
...
yao_age_cats$Sex <- relevel(as.factor(yao_age_cats$Sex), sex_ref)
yao_age_cats$age_cat <- relevel(yao_age_cats$age_cat, age_ref)
yao_age_cats$week <- relevel(as.factor(yao_age_cats$week), wk_ref)
