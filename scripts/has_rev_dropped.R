...
t_all <- 
  yao_hhld %>% 
  mutate(all_respondents = "All household heads") %>%
  count_and_prop_impact("has_rev_dropped", "all_respondents")

t_sex <- 
  yao_hhld %>% 
  count_and_prop_impact("has_rev_dropped", "cat_sex")
...
t_age <- 
  ...
  count_and_prop_impact("has_rev_dropped", "cat_age")
...
t_educ <- 
  ...
  count_and_prop_impact("has_rev_dropped", "cat_educ") %>% 
...
t_occup <-
  ...
  count_and_prop_impact("has_rev_dropped", "mcat_occup") %>% 
...
t_respect_measures <- 
  ...
  count_and_prop_impact("has_rev_dropped", "is_respecting_distancing")


t_pos <- 
  yao_hhld %>% 
  count_and_prop_impact("has_rev_dropped", "cat_pos")
...
t_combined_wide <-
  ...
  pivot_wider(id_cols = c(var, filler), names_from = has_rev_dropped, values_from = c(n, pct, pct_scale, pct_n, too_small, hjust)) %>%
...
has_rev_dropped_plot <- 
  ...
  labs(x = "",y = "",
       title = '<br>**Income decrease during the pandemic** <br> <span style = "font-size:10pt; color:#595959;">*"Since March 1st, has the revenue of the household fallen?"* </span>',
       caption = ""
  )
