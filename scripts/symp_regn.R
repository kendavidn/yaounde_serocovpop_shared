...
# regression is only run on folks who are seropositive
yao_regn <- 
  yao %>% 
...
all_regn <-
  yao_regn %>%
  glm(formula = paste("has_COVID_symptoms_num ~", signif_vars),
      data = .,
      family = "binomial") %>% 
  broom.mixed::tidy() %>%
  filter(row_number()!= 1 ) %>%   # drop intercept row
...
table_sec1 <-
  ...
  annotate("text", x = 4, y = max(all_sections$rowid) + 1, label = "Symp.", hjust = 0.5, fontface = "bold") +
  theme_void() +
...
symptomatic_regn_plot <- patchwork::wrap_plots(table_sec1,
                                            symptomatic_pct_plot, 
                                             univariate_odds_tab,
                                             univariate_odds_plot, 
                                             multivariate_odds_tab,
