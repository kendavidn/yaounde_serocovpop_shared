...
count_and_prop_distancing <-
  function(df, count_col, group_col) { 
...
yao_distance <- 
  yao %>% 
  mutate(is_respecting_distancing = recode(is_respecting_distancing, 
                                           "Definitely yes" = "Yes", 
                                           "Partly" = "Partly", 
                                           "Definitely not" = "No"))
...
t_all <- 
  ...
  count_and_prop_distancing("is_respecting_distancing", "all_respondents")

t_sex <- 
  yao_distance %>% 
  count_and_prop_distancing("is_respecting_distancing", "cat_sex")
...
t_age <- 
  ...
  count_and_prop_distancing("is_respecting_distancing", "cat_age")
...
t_educ <- 
  ...
  count_and_prop_distancing("is_respecting_distancing", "cat_educ") %>% 
  filter(var != "Other level")


t_occup <-
  yao_distance %>%
  separate_rows(mcat_occup, sep = "--") %>%
  mutate(mcat_occup = fct_lump(mcat_occup, prop = 0.05, other_level = "Other occupation")) %>% # space before Other so it's unique after combining
  count_and_prop_distancing("is_respecting_distancing", "mcat_occup") %>% 
  filter(var != "Other occupation")
...
t_revenue_drop <- 
  ...
  count_and_prop_distancing("is_respecting_distancing", "has_rev_dropped") %>% 
  filter(var != "Not applicable")

t_pos <- 
  yao_distance %>% 
  count_and_prop_distancing("is_respecting_distancing", "cat_pos")
...
t_combined_wide <-
  ...
  bind_rows(t_occup) %>%
  bind_rows(data.frame(var = "Fall in income during pandemic", filler = TRUE)) %>%
  bind_rows(t_revenue_drop) %>%
  bind_rows(data.frame(var = "Antibody test result", filler = TRUE)) %>%
  bind_rows(t_pos) %>%
  mutate(filler = replace_na(filler, FALSE)) %>% 
  pivot_wider(id_cols = c(var, filler), names_from = is_respecting_distancing, values_from = c(n, pct, pct_scale, pct_n, too_small, hjust)) %>%
...
follow_distancing_rules_plot <- 
  ...
  ## Columns
  # Yes
  annotate("text", y = nrow(t_combined_wide) + top_buffer, x = label_space + 0, label = 'Definitely', size = 3, color = my_darkgreen, hjust = 0, fontface = "bold") +
  geom_segment(aes(y = var, yend = var, x = label_space + 0, xend = label_space + 0 + pct_scale_Yes), size = seg_size, color = my_green) +
  geom_richtext(aes(y = var, x = label_space + 0 + pct_scale_Yes, label = pct_n_Yes), hjust = 1.1, color = "white", size = richtext_size, lineheight = 0, fill = "transparent", label.colour = NA, label.padding = unit(c(0.2, 0, 0.1, 0), "lines")) +
  # Partly
  annotate("text", y = nrow(t_combined_wide) + top_buffer, x = label_space + 40 , label = 'Partly', size = 3, color = my_green, hjust = 0, fontface = "bold") +
  geom_segment(aes(y = var, yend = var, x = label_space + pct_scale_Yes, xend = label_space + pct_scale_Yes + pct_scale_Partly), size = seg_size, color = my_lightgreen) +
  geom_richtext(aes(y = var, x = label_space + pct_scale_Yes + pct_scale_Partly, label = pct_n_Partly, hjust = hjust_Partly), color = my_verydarkgreen, size = richtext_size, lineheight = 0, fill = "transparent", label.colour = NA, label.padding = unit(c(0.2, 0, 0.1, 0), "lines")) +
  # # No
  #annotate("text", y = nrow(t_combined_wide) + top_buffer, x = label_space + 70 , label = 'Partly', size = 3, color = my_orange, hjust = 0, fontface = "bold") +
  #geom_segment(aes(y = var, yend = var, x = label_space + pct_scale_Yes + pct_scale_Partly, xend = label_space + pct_scale_Yes + pct_scale_Partly + pct_scale_No), size = seg_size, color = my_orange) +
  #geom_richtext(aes(y = var, x = label_space + pct_scale_Yes + pct_scale_Partly, label = pct_n_Partly, hjust = hjust_Partly), color = my_verydarkgreen, size = richtext_size, lineheight = 0, fill = "transparent", label.colour = NA, label.padding = unit(c(0.2, 0, 0.1, 0), "lines")) +
  # # No
  ...
  theme(axis.text.y = element_blank(),
        ...
        plot.title = element_textbox_simple(size = 14, lineheight = 1, 
  ...
  labs(x = "",y = "",
       title = '<br>**Respect of distancing measures** <br> <span style = "font-size:10pt; color:#595959;">*"Do you follow social distancing recommendations (avoid handshakes or hugs, avoid unnecessary outings, etc.)"* </span>',
       caption = ""
  )
